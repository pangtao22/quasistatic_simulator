...
class QuasistaticSimulator:
    def __init__(self, problem_definition: Dict,
                 visualize=False, is_quasi_dynamic=False):
    ...
    def step_lcp(self, q, q_a_cmd):
        ...
        tau_a = self.Jn_a.T.dot(P_n) + self.Jf_a.T.dot(P_f) + \
    ...
    def step_anitescu(self, q, q_a_cmd):
        ...
        prog.AddQuadraticCost(self.Kq_a * self.h**2,
                              -self.Kq_a.dot(dq_a_cmd) * self.h, v_a)
        ...
        constraints = prog.AddLinearConstraint(
            J, -phi_constraints / self.h,
            np.full_like(phi_constraints, np.inf), v)
