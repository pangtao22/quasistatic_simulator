...
MatrixXd CreateRandomMatrix(int n_rows, int n_cols, std::mt19937* gen) {
  ...
  return MatrixXd::NullaryExpr(n_rows, n_cols, [&]() {
    return dis(*gen);
  });
}
...
class TestBatchQuasistaticSimulator : public ::testing::TestWithParam<bool> {
 protected:
  ...
  // TODO(pang): simplify these setup functions with QuasistaticParser.
...
TEST_P(TestBatchQuasistaticSimulator, TestBundledBTrj) {
  ...
  // TODO(pang): change the free solver tolerance to a smaller number (1e-8?)
  //  once Tobia's better solver is supported.
