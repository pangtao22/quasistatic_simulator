...
class LogBarrierSolverBase {
 public:
  ...
  virtual void CalcGradientAndHessian(
  ...
  void Solve(const Eigen::Ref<const Eigen::MatrixXd>& Q,
             const Eigen::Ref<const Eigen::VectorXd>& b,
             const Eigen::Ref<const Eigen::MatrixXd>& G,
             const Eigen::Ref<const Eigen::VectorXd>& e, const double kappa_max,
             const bool use_free_solver, Eigen::VectorXd* v_star_ptr) const;
...
class SocpLogBarrierSolver : public LogBarrierSolverBase {
 public:
  ...
  template <class T>
  static drake::Vector3<T> CalcWi(
      const Eigen::Ref<const drake::Matrix3X<T>>& G_i, const T e_i,
      const Eigen::Ref<const drake::VectorX<T>>& v);
};

template <class T>
drake::Vector3<T> SocpLogBarrierSolver::CalcWi(
    const Eigen::Ref<const drake::Matrix3X<T>>& G_i, const T e_i,
    const Eigen::Ref<const drake::VectorX<T>>& v) {
